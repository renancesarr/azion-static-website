# Backlog (primeiras 100 tarefas) — MVP Azion/Context7 via MCP
_Data: 2025-10-18_

|ID|Épico|Área|Tarefa|Descrição / Critérios de Aceite|Inputs|Outputs / Artefatos|Prioridade|Status|Estimativa(h)|Dependências|
|---|---|---|---|---|---|---|---|---|---|---|
|TASK-001|Fundação|Ambiente|Definir objetivo do MVP|Escopo: storage, edge app, rules, domain, WAF, orquestrador; DoD estabelecido.|Documento de escopo|mvp_scope.md|P0|A Fazer|1|—|
|TASK-002|Fundação|Ambiente|Criar repositório local do MCP|Repositório azion-mcp com estrutura mínima (src/, dist/, .mcp-state/)|—|Estrutura de pastas criada|P0|A Fazer|0.5|—|
|TASK-003|Fundação|Ambiente|Configurar Node 18+|Node funcional; `node -v` ≥ 18|—|Node instalado|P0|A Fazer|0.5|—|
|TASK-004|Fundação|Ambiente|Inicializar package.json mínimo|`npm init -y`; scripts build/dev/start definidos|—|package.json|P0|A Fazer|0.5|—|
|TASK-005|Fundação|Ambiente|Adicionar TypeScript & SDK MCP|Instalar `typescript`, `@modelcontextprotocol/sdk`, `tsx`|—|deps instaladas|P0|A Fazer|0.5|TASK-001|
|TASK-006|Fundação|Ambiente|Criar tsconfig.json|Compilar para dist/ com ModuleResolution NodeNext|—|tsconfig.json|P0|A Fazer|0.5|TASK-004|
|TASK-007|Fundação|Ambiente|Criar pasta .mcp-state|Local de estado/artefatos JSON/LOG|—|.mcp-state/|P0|A Fazer|0.2|TASK-002|
|TASK-008|Fundação|Segurança|Definir variáveis de ambiente|AZION_TOKEN, CONTEXT7_API, UPLOAD_CONCURRENCY|—|.env.example (sem segredos)|P0|A Fazer|0.5|TASK-001|
|TASK-009|Fundação|Docs|Criar README base|Como rodar build/dev; visão geral; risco/limitações|—|README.md|P1|A Fazer|1|TASK-001|
|TASK-010|MCP|Servidor|Implementar bootstrap via STDIO|`src/server.ts` com `Server`+`StdioServerTransport` e registro de tools|—|src/server.ts|P0|A Fazer|1|TASK-004,TASK-005|
|TASK-011|MCP|Servidor|Script de build e start|`npm run build` compila; `npm run start` inicia loop MCP|—|dist/server.js|P0|A Fazer|0.5|TASK-010|
|TASK-012|MCP|Servidor|Health check básico|Log inicial e encerramento; erro claro se faltam envs|—|stdout logs|P0|A Fazer|0.3|TASK-010|
|TASK-013|MCP|Servidor|Tratamento de exceções globais|Capturar unhandledRejection/uncaughtException|—|logger central|P1|A Fazer|0.5|TASK-010|
|TASK-014|Utils|HTTP|Função required(env)|Falhar cedo se env ausente|AZION_TOKEN, CONTEXT7_API|Erro amigável|P0|A Fazer|0.3|TASK-008|
|TASK-015|Utils|HTTP|Função http(method,path,body,headers)|fetch nativo; Authorization Token; Accept JSON; erros ricos|method,path,body|JSON/Erro detalhado|P0|A Fazer|1|TASK-013|
|TASK-016|Utils|MIME|Mapeador MIME|Inferir Content-Type por extensão (.html,.css,.js,.json,.png,.jpg,.svg,.webp,.ico,.map)|extensão|string MIME|P0|A Fazer|1|TASK-013|
|TASK-017|Utils|Compressão|Detecção gzip|Se nome termina .gz, setar Content-Encoding=gzip; derivar MIME pela base sem .gz|nome arquivo|headers corretos|P0|A Fazer|0.5|TASK-016|
|TASK-018|Utils|FS|Walk de diretório|Listar arquivos recursivamente; normalizar caminhos POSIX|local_dir|lista de paths|P0|A Fazer|0.8|TASK-013|
|TASK-019|Utils|Hash|SHA256 de arquivo|Calcular hash p/ idempotência de upload|arquivo|hex sha256|P1|A Fazer|0.8|TASK-017|
|TASK-020|Utils|Concorrência|Pool de promessas|Executor com limite (4–8) e retries simples|lista de jobs|resultados|P1|A Fazer|1|TASK-017|
|TASK-021|Storage|Bucket|Tool azion.create_bucket|POST /v4/storage/buckets; edge_access default read_only; salvar storage_bucket.json|.name,.edge_access|.mcp-state/storage_bucket.json|P0|A Fazer|1|TASK-014|
|TASK-022|Storage|Bucket|Idempotência create_bucket|Se já existe, retornar estado/ID existente|name|JSON coerente|P0|A Fazer|0.8|TASK-021|
|TASK-023|Storage|Upload|Tool azion.put_object|POST objetos; body binário; respeitar content_type/encoding|bucket,object_path,content_b64|JSON da API|P0|A Fazer|1.2|TASK-016,TASK-017|
|TASK-024|Storage|Upload|Erros de upload verbosos|Incluir status, body truncado, dica de causa|—|Mensagens claras|P0|A Fazer|0.5|TASK-023|
|TASK-025|Storage|Upload|Tool azion.upload_dir|Walk; inferência MIME; .gz → encoding; concurrency; dry_run; index.json (hash)|bucket,local_dir,strip_gz?,concurrency?,dry_run?|uploads.log, index.json|P0|A Fazer|3|TASK-018,TASK-019,TASK-020,TASK-024|
|TASK-026|Storage|Upload|Idempotência por hash|Pular upload se sha256 igual salvo em index.json|object_path,sha|uploads otimizados|P1|A Fazer|1|TASK-026|
|TASK-027|Storage|Upload|Relatório final de upload|Resumo: enviados, pulados, falhas, duração|—|stdout + uploads.log|P1|A Fazer|0.5|TASK-026|
|TASK-028|Edge App & Rules|App|Tool azion.create_edge_application|Cria app ativa com modules.edge_cache=true (edge_functions opcional)|name|edge_application.json|P0|A Fazer|1|TASK-013|
|TASK-029|Edge App & Rules|Connector|Tool azion.create_edge_connector|type=edge_storage, attributes:{bucket,prefix:'/'}|name,bucket,prefix?|edge_connector.json|P0|A Fazer|1|TASK-030,TASK-022|
|TASK-030|Edge App & Rules|Rules|Tool azion.add_origin_rule|criteria ${uri} starts_with '/'; behavior set_edge_connector|edge_application_id,connector_id|rule_origin.json|P0|A Fazer|1|TASK-031|
|TASK-031|Edge App & Rules|Rules|Validação regra de origem|GET rules e confirmar behavior/value esperado|edge_application_id|validation_rules.md|P1|A Fazer|0.8|TASK-032|
|TASK-032|DNS/Domain|Domain|Tool azion.create_domain|Cria workload/domain ativo p/ custom domain; HTTP/2 on|domain_name,protocols?|edge_domain.json (workload_id)|P0|A Fazer|1.5|TASK-029|
|TASK-033|DNS/Domain|Deploy|Tool azion.deploy_domain_to_app|Cria deployment current/active=true p/ app|workload_id,edge_application_id|deployment.json|P0|A Fazer|1|TASK-036,TASK-033|
|TASK-034|DNS/Domain|DNS|Tool azion.dns_instructions|Gera instruções CNAME (ou nota se Azion DNS)|workload_id ou endpoint|dns_instructions.txt|P1|A Fazer|0.5|TASK-037|
